...
class Retinaface(object):
    ...
    #   初始化Retinaface
    # ---------------------------------------------------#
    def __init__(self, encoding=0, npy_save_path="", pth_save_path="", device=False, **kwargs):
        ...
        try:
            ...
            self.known_face_names = np.load(
                "{npy_save_path}/{backbone}_names.npy".format(backbone=self.facenet_backbone,
                                                              npy_save_path=npy_save_path))
        except:
            if not encoding:
    ...
    def generate(self):
        ...
        device = torch.device(self.cuda if self.cuda else 'cpu')
        ...
        if self.cuda:
            self.net = nn.DataParallel(self.net,device_ids=self.device_num)
            self.net = self.net.cuda(self.cuda)

            self.facenet = nn.DataParallel(self.facenet,device_ids=self.device_num)
            self.facenet = self.facenet.cuda(self.cuda)
        print('Finished!')

    def encode_face_dataset(self, image_paths, names, npy_save_path):
        face_encodings = []
        for index, path in enumerate(tqdm(image_paths)):
            ...
            with torch.no_grad():
                ...
                if self.cuda:
                    image = image.cuda(self.cuda)
                    anchors = anchors.cuda(self.cuda)
            ...
            with torch.no_grad():
                ...
                if self.cuda:
                    crop_img = crop_img.cuda(self.cuda)

                face_encoding = self.facenet(crop_img)[0].cpu().numpy()
                face_encodings.append(face_encoding)

        np.save("{npy_save_path}/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone,
                                                                      npy_save_path=npy_save_path), face_encodings)
        np.save(
            "{npy_save_path}/{backbone}_names.npy".format(backbone=self.facenet_backbone, npy_save_path=npy_save_path),
            names)

    # ---------------------------------------------------#
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image, facenet_threhold):
        # ---------------------------------------------------#
        #  对输入图像进行一个备份，后面用于绘图
        # ---------------------------------------------------#
        old_image = image.copy()
        ...
        # ---------------------------------------------------#
        time2 = time.time()
        #logging.debug('Retinaface代码用时:{}'.format(time2-time1))
        ...
        # -----------------------------------------------#
        time3 = time.time()
        #logging.debug('faceNET代码用时:{}'.format(time3-time2))
        # -----------------------------------------------#
        #   人脸特征比对-开始
        # -----------------------------------------------#
        ...
        time4 = time.time()
        #logging.debug('人脸对比代码用时:{}'.format(time4-time3))
        # -----------------------------------------------#
        #   人脸特征比对-结束
        # -----------------------------------------------#
        list_append = []
        ...
        for i in list_append:
            ...
            #   如果不是必须，可以换成cv2只显示英文。
            # --------------------------------------------------------------#
            old_image = cv2ImgAddText(old_image, name, b[0] + 5, b[3] - 25, (255, 255, 255))
            """ for i, b in enumerate(boxes_conf_landms):
                ...
                old_image = cv2ImgAddText(old_image, name, b[0] + 5, b[3] - 25, (255, 255, 255))"""
        time5 = time.time()
        #logging.debug('画框添加名字代码用时:{}'.format(time5-time4))
        return old_image, face_names

    def get_FPS(self, image, test_interval, facenet_threhold):
        # ---------------------------------------------------#
        #   对输入图像进行一个备份，后面用于绘图
        # ---------------------------------------------------#
        old_image = image.copy()
